...
COPY ["backend/src/Presentation/LLMProxy.Admin.API/LLMProxy.Admin.API.csproj", "Presentation/LLMProxy.Admin.API/"]
...
RUN dotnet restore "Presentation/LLMProxy.Admin.API/LLMProxy.Admin.API.csproj"

# Copy backend source and build
COPY backend/src/ .
WORKDIR "/src/Presentation/LLMProxy.Admin.API"
RUN dotnet build "LLMProxy.Admin.API.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "LLMProxy.Admin.API.csproj" -c Release -o /app/publish /p:UseAppHost=false

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "LLMProxy.Admin.API.dll"]
